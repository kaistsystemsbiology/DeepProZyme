...
if __name__ == '__main__':
    ...
    input_seqs = input_seqs * 10
    input_ecs = input_ecs * 10
    ...
    mask = torch.zeros([len(explainECs), len(thirdECs)]).bool()
    for i, ec_4 in enumerate(explainECs):
        for j, ec_3 in enumerate(thirdECs):
            if p.match(ec_4).group(1) == ec_3:
                mask[i][j] = 1
