...
class DeepContactECDataset(Dataset):
    def __init__(self, data_X, data_ec, data_map, explainECs, tokenizer_name='Rostlab/prot_bert_bfd', max_length=1000, pred=False):
        self.tokenizer = AutoTokenizer.from_pretrained(tokenizer_name, do_lower_case=False)
        self.max_length = max_length
        self.data_X = data_X
        self.data_Y = data_ec
    ...
    def __getitem__(self, idx):
        ...
        dist_map = torch.tensor(self.data_map[idx]) < 8
        seq_len = len(dist_map)
        cnt_map = torch.zeros(self.max_length, self.max_length)
        cnt_map[1:seq_len+1, 1:seq_len+1] = dist_map
        
        sample['maps'] = cnt_map.view(-1)
        return sample



class PSSMDataset(Dataset):
