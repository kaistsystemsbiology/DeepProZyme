...
from deepec.data_loader import PSSMDataset, DeepECDataset
...
if __name__ == '__main__':
    ...
    pssm_data = np.load(input_data_file)
    input_ids = np.array(list(pssm_data.keys()))
    data_Y = {}
    with open('./Dataset/pssm_data/SwissProt_3_1000_labels.txt', 'r') as fp:
        while True:
            line = fp.readline()
            if not line: break
            seq_id, ecs = line.strip().split('\t')
            data_Y[seq_id] = ecs.split(';')
    ...
    logging.info(f'Number of sequences used- Train: {len(train_ids)}')
    logging.info(f'Number of sequences used- Validation: {len(val_ids)}')
    logging.info(f'Number of sequences used- Test: {len(test_ids)}')

    explainECs = []
    for ecs in data_Y.values():
        explainECs += ecs
    explainECs = list(set(explainECs))
    explainECs.sort()
    logging.info(f'Number of EC number types: {len(explainECs)}')

    train_ec_types = []
    for seq_id in train_ids:
        train_ec_types += data_Y[seq_id]
    len_train_ecs = len(set(train_ec_types))

    val_ec_types = []
    for seq_id in val_ids:
        val_ec_types += data_Y[seq_id]
    len_val_ecs = len(set(val_ec_types))
    
    test_ec_types = []
    for seq_id in test_ids:
        test_ec_types += data_Y[seq_id]
    ...
    trainDataset = PSSMDataset(data_X=train_ids, data_Y=data_Y, pssm_data=pssm_data, explainECs=explainECs)
    valDataset = PSSMDataset(data_X=train_ids, data_Y=data_Y, pssm_data=pssm_data, explainECs=explainECs)
    testDataset = PSSMDataset(data_X=train_ids, data_Y=data_Y, pssm_data=pssm_data, explainECs=explainECs)
    ...
    model = PSSMBert(config, out_features=explainECs)
